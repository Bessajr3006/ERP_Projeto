(() => {
    ...
    async function main() {
        ...
        let itemsCount = 0;
        const els = {
            tableBody: getEl('templatesTable'),
            btnOpenModal: getEl('btnOpenModal'),
            templateModal: getEl('templateModal'),
            templateModalBackdrop: getEl('templateModalBackdrop'),
            btnCancelTemplate: getEl('btnCancelTemplate'),
            templateForm: getEl('templateForm'),
            btnAddItem: getEl('btnAddItem'),
            itemsContainer: getEl('itemsContainer')
        };
        if (!els.tableBody || !els.templateForm)
            return;
        ...
        function renderView() {
            ...
            els.tableBody.innerHTML = templates.map((tpl) => `
        ...
        function setupListeners() {
            ...
            els.templateForm?.addEventListener('submit', async (e) => {
                e.preventDefault();
                const id = getEl('templateId')?.value;
                // Collect Items
                const itemDivs = els.itemsContainer?.querySelectorAll('.item-row');
                const items = [];
                let hasError = false;
                itemDivs?.forEach(div => {
                    const debitId = div.querySelector('.debit-select')?.value;
                    const creditId = div.querySelector('.credit-select')?.value;
                    const historyTpl = div.querySelector('.history-input')?.value;
                ...
                const payload = {
                    code: getEl('templateCode')?.value || '',
                    description: getEl('templateDescription')?.value || '',
                    active: getEl('templateActive')?.checked ? 1 : 0,
                    items
                };
                try {
                    if (id) {
        ...
        function getAccountOptions(selectedValue = '') {
            const options = accountsList.map((a) => `<option value="${a.public_id}" ${a.public_id === selectedValue ? 'selected' : ''}>${a.code} - ${a.name}</option>`).join('');
        ...
        function openModal(id = null) {
            ...
            const idInput = getEl('templateId');
            if (idInput)
                idInput.value = '';
            if (title)
                title.textContent = 'Novo Lançamento Automático';
            if (els.itemsContainer)
                els.itemsContainer.innerHTML = '';
            getEl('templateActive').checked = true;
            if (id) {
                const tpl = templates.find((t) => t.public_id === id);
                if (tpl) {
                    if (idInput)
                        idInput.value = tpl.public_id;
                    if (title)
                        title.textContent = 'Editar Lançamento Automático';
                    getEl('templateCode').value = tpl.code;
                    getEl('templateDescription').value = tpl.description;
                    getEl('templateActive').checked = !!tpl.active;
                    if (tpl.items && Array.isArray(tpl.items)) {
                        tpl.items.forEach((item) => renderItemRow(item));
                    }
                }
            }
            else {
        ...
        async function deleteTemplate(id) {
            if (!confirm('Deseja excluir este Lançamento Automático?'))
                return;
            try {
