...
(() => {
    ...
    let editingId = null;
    // UI Elements
    const historiesTable = document.getElementById('historiesTable');
    ...
    async function loadHistories() {
        try {
            if (historiesTable)
                historiesTable.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-gray-500 dark:text-gray-400">Carregando...</td></tr>';
            ...
            if (historiesTable)
                historiesTable.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-red-500">Erro ao carregar dados.</td></tr>';
            console.error(e);
        }
    }
    ...
    function openNewModal() {
        ...
        if (historyModalTitle)
            historyModalTitle.textContent = 'Novo Histórico Automático';
        if (historyForm)
            historyForm.reset();
        if (historyModal)
    ...
    async function openEditModal(id) {
        ...
        editingId = id;
        if (historyModalTitle)
            historyModalTitle.textContent = 'Editar Histórico Automático';
        try {
            const data = await api(`/accounting/histories/${id}`);
            const h = data.data;
            document.getElementById('historyId').value = h.public_id;
            document.getElementById('historyCode').value = h.code;
            document.getElementById('historyDescription').value = h.description;
            document.getElementById('historyText').value = h.history_text;
            document.getElementById('historyActive').checked = !!h.active;
            if (historyModal)
                historyModal.classList.remove('hidden');
        }
        catch (e) {
    ...
    async function deleteHistory(id) {
        ...
        try {
            ...
            // @ts-ignore
            if (typeof Swal !== 'undefined')
                Swal.fire('Excluído!', 'O histórico foi removido.', 'success');
            loadHistories();
        }
        catch (e) {
            ...
            // @ts-ignore
            if (typeof Swal !== 'undefined')
                Swal.fire('Erro', e.message || 'Falha ao excluir. Pode estar em uso.', 'error');
        }
    }
    // Listeners
    if (btnOpenModal)
        btnOpenModal.addEventListener('click', openNewModal);
    if (btnCancelHistory)
        btnCancelHistory.addEventListener('click', closeModal);
    if (historyModalBackdrop)
        historyModalBackdrop.addEventListener('click', closeModal);
    if (historyForm) {
        historyForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const history_text = document.getElementById('historyText').value.trim();
                const active = document.getElementById('historyActive').checked;
                const payload = {
                    code,
                    description,
                    history_text,
                ...
                if (isEditing && editingId) {
                    ...
                    // @ts-ignore
                    if (typeof Swal !== 'undefined')
                        Swal.fire('Sucesso', 'Histórico atualizado com sucesso!', 'success');
                }
                else {
                    ...
                    // @ts-ignore
                    if (typeof Swal !== 'undefined')
                        Swal.fire('Sucesso', 'Novo histórico criado com sucesso!', 'success');
                }
                closeModal();
                loadHistories();
            }
                ...
                // @ts-ignore
                if (typeof Swal !== 'undefined')
                    Swal.fire('Erro', error.message || 'Falha ao salvar as informações.', 'error');
                else
                    alert(error.message || 'Falha ao salvar as informações.');
            }
            finally {
                if (btnSaveHistory) {
