(() => {
    ...
    async function main() {
        ...
        // --- Elements ---
        const els = {
            tableBody: getEl('entriesTable'),
            selectAllCheckbox: getEl('selectAllCheckbox'),
            batchActions: getEl('batchActions'),
            btnBatchDelete: getEl('btnBatchDelete'),
            batchDeleteCount: getEl('batchDeleteCount'),
            filterBody: getEl('filterBody'),
            toggleFilterBtn: getEl('toggleFilterBtn'),
            filterChevron: getEl('filterChevron'),
            filterForm: getEl('filterForm'),
            filterStartDate: getEl('filterStartDate'),
            filterEndDate: getEl('filterEndDate'),
            filterAccountGroup: getEl('filterAccountGroup'),
            filterAccountId: getEl('filterAccountId'),
            filterSearch: getEl('filterSearch'),
            btnClearFilters: getEl('btnClearFilters'),
            btnClearFiltersKeepDate: getEl('btnClearFiltersKeepDate'),
            btnListView: getEl('btnListView'),
            btnGridView: getEl('btnGridView'),
            btnOpenModal: getEl('btnOpenModal'),
        ...
        if (els.filterBody && els.toggleFilterBtn) {
            ...
            if (els.filterChevron) {
                els.filterChevron.style.transform = 'rotate(-90deg)';
            }
            els.toggleFilterBtn.addEventListener('click', () => {
                ...
                if (els.filterChevron) {
                    els.filterChevron.style.transform = filterIsOpen ? 'rotate(0deg)' : 'rotate(-90deg)';
                }
            });
        }
        ...
        async function loadRelations() {
            try {
                ...
                if (res.data) {
                    ...
                    accountsList = res.data.filter((a) => a.type === 'analytic' && a.status === 'active');
        ...
        function getFilteredEntries() {
            ...
            if (filters.startDate)
                data = data.filter((e) => String(e.entry_date).split('T')[0] >= filters.startDate);
            if (filters.endDate)
                data = data.filter((e) => String(e.entry_date).split('T')[0] <= filters.endDate);
            if (filters.accountId) {
                data = data.filter((e) => String(e.debit_account_code).startsWith(filters.accountId) || String(e.credit_account_code).startsWith(filters.accountId));
            }
            else if (filters.accountGroup) {
                data = data.filter((e) => String(e.debit_account_code).startsWith(filters.accountGroup) ||
        ...
        function renderView() {
            ...
            if (filtered.length === 0) {
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td class="max-w-50 px-2 py-3 text-xs line-clamp-2 text-gray-700 dark:text-gray-300">${entry.history}</td>
                    <td class="px-2 py-3 whitespace-nowrap text-center text-sm font-medium">
                        <div class="flex items-center justify-center space-x-3">
                            <button type="button" class="btn-edit text-brand-600 hover:text-brand-900 dark:text-brand-400 dark:hover:text-brand-300 transition-colors" data-id="${entry.public_id}" title="Editar">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </button>
                            <button type="button" class="btn-delete text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 transition-colors" data-id="${entry.public_id}" title="Excluir">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            `)
                    .join('');
                // Row listeners
                els.tableBody.querySelectorAll('.row-checkbox').forEach((cb) => {
                    cb.addEventListener('change', (e) => {
                        const t = e.target;
                        if (t.checked)
                            selectedIds.add(t.value);
                        else
                            selectedIds.delete(t.value);
                        updateBatchActions();
                    });
                });
        ...
        function updateFooter(filtered) {
            if (els.footerCount)
                els.footerCount.textContent = String(filtered.length);
            const total = filtered.reduce((s, e) => s + (parseFloat(e.amount) || 0), 0);
            if (els.footerTotalDebit)
                els.footerTotalDebit.textContent = formatCurrency(total);
            if (els.footerTotalCredit)
                els.footerTotalCredit.textContent = formatCurrency(total);
        }
        function updateBatchActions() {
            if (!els.batchActions || !els.batchDeleteCount)
                return;
        ...
        function populateAccountDropdowns() {
            const debitSelect = getEl('debitAccountId');
            const creditSelect = getEl('creditAccountId');
            const options = accountsList
                .map((a) => `<option value="${a.public_id}">${a.code} - ${a.name}</option>`)
                .join('');
            if (debitSelect)
                debitSelect.innerHTML = '<option value="">Selecione...</option>' + options;
            if (creditSelect)
                creditSelect.innerHTML = '<option value="">Selecione...</option>' + options;
        ...
        function setupListeners() {
            ...
            els.btnClearFiltersKeepDate?.addEventListener('click', () => {
                if (els.filterAccountGroup)
                    els.filterAccountGroup.value = '';
                if (els.filterAccountId)
                    els.filterAccountId.value = '';
                if (els.filterSearch)
            ...
            els.selectAllCheckbox?.addEventListener('change', (e) => {
                ...
                const t = e.target;
                if (t.checked) {
                    filtered.forEach((entry) => selectedIds.add(entry.public_id));
                }
                else {
                    selectedIds.clear();
                }
            ...
            els.entryForm?.addEventListener('submit', async (e) => {
                e.preventDefault();
                const amountRaw = parseFloat(getEl('entryAmount')?.value || '0');
                if (!amountRaw || amountRaw <= 0) {
                    alert('Informe um valor positivo para o lançamento.');
                    return;
                }
                const payload = {
                    entry_date: getEl('entryDate')?.value || '',
                    document_ref: getEl('documentRef')?.value || '',
                    debit_account_id: getEl('debitAccountId')?.value || '',
                    credit_account_id: getEl('creditAccountId')?.value || '',
                    amount: amountRaw,
                    history: getEl('entryHistory')?.value || '',
                };
                const id = getEl('entryId')?.value || '';
                try {
                    if (id) {
        ...
        function openModal(id = null) {
            els.entryForm?.reset();
            const entryId = getEl('entryId');
            const entryDate = getEl('entryDate');
            ...
            if (id) {
                const entry = entries.find((e) => e.public_id === id);
                if (entry) {
                    if (entryId)
                        entryId.value = entry.public_id;
                    if (entryDate)
                        entryDate.value = String(entry.entry_date).split('T')[0];
                    const doc = getEl('documentRef');
                    const debit = getEl('debitAccountId');
                    const credit = getEl('creditAccountId');
                    const amount = getEl('entryAmount');
                    const history = getEl('entryHistory');
                    if (doc)
                        doc.value = entry.document_ref || '';
                    if (debit)
                        debit.value = entry.debit_account_public_id;
                    if (credit)
                        credit.value = entry.credit_account_public_id;
                    if (amount)
        ...
        function setupImportLogic() {
            getEl('importFile')?.addEventListener('change', (e) => {
                ...
                reader.onload = (evt) => {
                    ...
                    if (importData.csvData.length > 0)
                        importData.csvHeaders = importData.csvData[0].map((h) => String(h).trim());
                };
                reader.readAsText(file, 'windows-1252');
            });
            getEl('btnNextImportStep')?.addEventListener('click', () => {
            ...
            getEl('btnExecuteImport')?.addEventListener('click', () => {
                void executeImport();
            });
        }
        ...
        async function executeImport() {
            ...
            const bar = getEl('importProgressBar');
            ...
            const hasHeader = !!getEl('importHasHeader')?.checked;
            const startIndex = hasHeader ? 1 : 0;
            const mappings = {
                entry_date: parseInt(getEl('map_entry_date')?.value || '-1', 10),
                document_ref: parseInt(getEl('map_document_ref')?.value || '-1', 10),
                debit_account_code: parseInt(getEl('map_debit_account_code')?.value || '-1', 10),
                credit_account_code: parseInt(getEl('map_credit_account_code')?.value || '-1', 10),
                amount: parseInt(getEl('map_amount')?.value || '-1', 10),
                history: parseInt(getEl('map_history')?.value || '-1', 10),
            };
            const payload = [];
            ...
            if (format === 'single') {
                for (let i = startIndex; i < importData.csvData.length; i++) {
                    ...
                    payload.push({
                        ...
                        credit_account_code: mappings.credit_account_code !== -1 ? String(row[mappings.credit_account_code]).trim() : '',
            ...
            if (payload.length === 0) {
                if (bar)
                    bar.style.width = '0%';
                if (txt)
                    txt.textContent = 'Erro: Nenhum lançamento válido.';
                return;
            }
            try {
                if (bar)
                    bar.style.width = '50%';
                if (txt)
                    txt.textContent = `Enviando ${payload.length} lançamentos...`;
                ...
                if (bar)
                    bar.style.width = '100%';
                if (txt)
                    txt.textContent = `Concluído! ${response.data?.success || 0} criados.`;
                // Add handler so close button refreshes data
                document.querySelectorAll('.btnCancelImport').forEach((b) => {
                    const old = b.onclick;
                    b.onclick = async () => {
                        if (old)
                            old.call(b);
