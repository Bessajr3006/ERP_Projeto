(() => {
    ...
    const resetFileDropzonesUI = () => {
        ['Cert', 'Key', 'WebhookCert', 'WebhookKey'].forEach((type) => {
            ...
            let fileInputId = '';
            if (type === 'Cert')
                fileInputId = 'fileCertificado';
            else if (type === 'Key')
                fileInputId = 'fileKey';
            else if (type === 'WebhookCert')
                fileInputId = 'fileWebhookCertificado';
            else if (type === 'WebhookKey')
                fileInputId = 'fileWebhookKey';
            const fileInput = getById(fileInputId);
            if (dropzone) {
                dropzone.classList.remove('hidden');
                dropzone.style.display = '';
        ...
        if (dropzone && filenameDiv && filenameText) {
            if (hasContent) {
                ...
                if (type === 'Cert')
                    ...
                    labelName = 'Certificado do Webhook .crt';
                else if (type === 'WebhookKey')
                    labelName = 'Chave do Webhook .key';
    ...
    const testApiConnectionDirectly = async (bankId, btn) => {
        const originalText = btn.innerHTML;
