    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        `;
            ...
            <div class="flex flex-col md:flex-row flex-1 min-h-0 w-full rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm overflow-hidden relative">
                ...
                .map((cat) => `
                                    <li class="shrink-0">
                                        <button type="button" data-cat="${cat}" class="cat-btn inline-block pb-3 px-1.5 border-b-2 transition-colors ${state.activeCategory === cat
                ? 'border-brand-600 text-brand-600'
                : 'border-transparent text-gray-500'}">${cat}</button>
                                    </li>
                                `)
                .map((html) => html.trim())
                .join('')}
