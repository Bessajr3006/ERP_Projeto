(function initCompaniesPage() {
    ...
    let companyDocMask;
    let companyPhoneMask;
    let companyZipMask;
    let companyIbgeStates = [];
    ...
    function populateCompanyStateOptions(selectedValue = '') {
        ...
        if (!stateSelect || !companyIbgeStates.length)
            return;
    ...
    function updateSummary(items) {
        ...
        if (elStates)
            elStates.textContent = String(states.size);
    }
    ...
    async function switchCompanyContext(publicId) {
        ...
        try {
            ...
            if (response && response.data && response.data.token) {
                ...
                if (company.cnpj)
                    localStorage.setItem('erp_last_company_cnpj', company.cnpj);
        ...
        qsa('#companiesTable .toggle-company-status-btn').forEach((button) => {
            button.addEventListener('click', () => {
                if (button.dataset.id)
                    toggleCompanyStatus(button.dataset.id);
            });
        });
        qsa('#companiesTable .switch-company-btn').forEach((button) => {
            button.addEventListener('click', () => {
                if (button.dataset.id)
                    switchCompanyContext(button.dataset.id);
            });
        });
