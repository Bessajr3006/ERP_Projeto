...
class CrudManager {
    ...
    async init() {
        ...
        if (this.filterConfig && filterPanel && typeof filterPanel.mount === 'function') {
            ...
            (this.filterConfig.fields || []).forEach((f) => {
    ...
    applyFilters() {
        ...
        if (this.applyFiltersFn) {
            ...
            if (filterPanel && typeof filterPanel.matchesSearch === 'function') {
                filtered = this.data.filter((item) => filterPanel.matchesSearch(item, ['name', 'description'], search));
            }
            else {
    ...
    _bindActionEvents() {
        document.querySelectorAll('.edit-btn').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                const item = rawItem ? JSON.parse(rawItem) : this.data.find((x) => x.public_id === id);
                if (this.onEdit)
                    this.onEdit(item);
            });
        });
        document.querySelectorAll('.duplicate-btn').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                const id = target.getAttribute('data-id');
                const rawItem = target.getAttribute('data-item');
                let item = rawItem ? JSON.parse(rawItem) : this.data.find((x) => x.public_id === id);
                item = { ...item, public_id: '' };
                if (this.onDuplicate) {
    ...
    _setupModals() {
        ...
        if (btnOpen) {
            btnOpen.addEventListener('click', () => {
                if (this.onEdit)
                    this.onEdit(null); // Pass null to signify new creation
            });
        }
