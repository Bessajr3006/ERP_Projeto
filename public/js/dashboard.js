...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderStatCard(title, value, color, link, linkText, svgPath, isAmountColor = false) {
        const colorClasses = {
    ...
    function renderCountCard(title, value, color, link, linkText, svgPath) {
        const colorClasses = {
    ...
    function getChartHtml() {
        ...
        let activeVals = [1];
        if (state.showIncome)
            activeVals = activeVals.concat(points.map((p) => p.income));
        if (state.showExpense)
            activeVals = activeVals.concat(points.map((p) => p.expense));
    ...
    async function loadData() {
        ...
        try {
            ...
            state.user = userRes?.data?.user || userRes?.data;
