...
(() => {
    ...
    const declarationsTable = document.getElementById('declarationsTable');
    ...
    function updateBatchActionsVisibility() {
        ...
        if (checkboxes.length > 0) {
            ...
            if (selectedCountSpan)
                selectedCountSpan.textContent = checkboxes.length.toString();
        }
        else {
    ...
    async function loadDeclarations() {
        ...
        if (!type) {
            ...
            if (typeof Swal !== 'undefined')
                Swal.fire('Aviso', 'Informe o tipo de declaração antes de buscar.', 'warning');
            return;
        }
        showLoading();
        try {
    ...
    function renderTable() {
        ...
        if (filterCustomer && filterCustomer.value) {
            ...
            filtered = currentDeclarations.filter(c => (c.customer_name || '').toLowerCase().includes(search) ||
        ...
        declarationsTable.querySelectorAll('.btn-delete').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                if (!confirm('Deseja realmente excluir esta declaração?'))
                    return;
                const target = e.currentTarget;
                const customerId = target.getAttribute('data-id');
                const monthOverride = target.getAttribute('data-month');
            ...
            input.addEventListener('change', async (e) => {
                ...
                if (customerId && file) {
                    try {
                        ...
                        // @ts-ignore
                        if (typeof Swal !== 'undefined')
                            Swal.fire('Erro', error.message || 'Falha ao fazer upload do arquivo.', 'error');
                        hideLoading();
                    }
                }
            });
    ...
    async function uploadReceipt(customerId, base64, monthOverride) {
        const month = monthOverride || filterMonth.value;
        const year = filterYear.value;
        const type = filterType.value.trim().toUpperCase();
        const payload = { receipt_base64: base64 };
        ...
        try {
            ...
            // @ts-ignore
            if (typeof Swal !== 'undefined')
                Swal.fire('Erro', error.message || 'Falha ao anexar comprovante.', 'error');
            hideLoading();
        }
    }
    async function updateStatus(customerId, status, promptForDate = false, monthOverride, skipReload = false) {
        const month = monthOverride || filterMonth.value;
        const year = filterYear.value;
        const type = filterType.value.trim().toUpperCase();
    ...
    async function deleteDeclaration(customerId, monthOverride, skipReload = false) {
        const month = monthOverride || filterMonth.value;
        const year = filterYear.value;
        const type = filterType.value.trim().toUpperCase();
        showLoading();
        try {
            ...
            // @ts-ignore
            if (typeof Swal !== 'undefined')
                Swal.fire('Erro', error.message || 'Falha ao excluir.', 'error');
            throw error;
        }
        finally {
            hideLoading();
        ...
        document.querySelectorAll('.batch-action-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (selectedCheckboxes.length === 0)
                    return;
                if (action === 'DELETE') {
                    if (!confirm('Tem certeza que deseja excluir as declarações selecionadas?'))
                        return;
                }
                showLoading();
                let hasErrors = false;
                ...
                if (!hasErrors) {
                    ...
                    if (typeof UI !== 'undefined' && UI.showAlert)
                        UI.showAlert('alertMessage', 'Lote processado com sucesso!', 'success');
                }
                else {
                    // @ts-ignore
                    if (typeof Swal !== 'undefined')
                        Swal.fire('Aviso', 'Alguns itens não puderam ser processados.', 'warning');
                }
                await loadDeclarations();
