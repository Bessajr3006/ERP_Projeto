(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Form Submit
        getEl('categoryForm')?.addEventListener('submit', handleSaveCategory);
        FilterPanel.mount({
    ...
    async function handleSaveCategory(e) {
        ...
        try {
            if (g_editingId) {
                await api(`/finance/categories/${g_editingId}`, {
                    method: 'PUT',
    ...
    window.deleteCategory = async (id) => {
        if (!confirm('Tem certeza que deseja excluir esta categoria? Ela pode estar vinculada a lançamentos financeiros.'))
            return;
        try {
            await api(`/finance/categories/${id}`, {
                method: 'DELETE',
            });
    ...
    function renderTable() {
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap">
                <input type="checkbox" class="item-checkbox h-4 w-4 text-brand-600 focus:ring-brand-500 border-gray-300 dark:border-slate-600 rounded cursor-pointer" value="${cat.public_id || ''}" data-bwignore="true" data-lpignore="true" placeholder="">
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500 dark:text-gray-400 font-mono">#${String(cat.id || '').padStart(4, '0')}</td>
        ...
        <div class="bg-white dark:bg-slate-800 shadow rounded-lg p-5 flex flex-col border border-gray-100 dark:border-slate-700 relative group">
            ...
            <div class="flex justify-between items-start mb-3">
                <div class="flex items-center z-10 pt-1">
                    ...
                    <span class="ml-2 text-xs font-mono font-medium text-gray-500 dark:text-gray-400">#${String(cat.id || '').padStart(4, '0')}</span>
                </div>
    ...
    function closeModal() {
        ...
        getEl('categoryForm')?.reset();
        g_editingId = null;
    }
    window.editCategory = (publicId) => {
        const cat = g_categories.find((c) => c.public_id === publicId);
        if (cat)
            openModal(cat);
    };
    window.duplicateCategory = (publicId) => {
        const cat = g_categories.find((c) => c.public_id === publicId);
        if (cat) {
            const dup = { ...cat, public_id: '' };
            openModal(dup);
        }
    };
    // --- Utils ---
