(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        getEl('typeForm')?.addEventListener('submit', handleSaveType);
        FilterPanel.mount({
    ...
    async function handleSaveType(e) {
        e.preventDefault();
        const name = getEl('typeName')?.value;
        const description = getEl('typeDescription')?.value;
        const data = {
            name: name,
        ...
        try {
            if (g_editingId) {
                await api(`/finance/category-types/${g_editingId}`, {
                    method: 'PUT',
                ...
                await api('/finance/category-types', {
                    method: 'POST',
                    body: JSON.stringify(data),
                });
    ...
    window.deleteType = async (id) => {
        if (!confirm('Tem certeza que deseja excluir este tipo de categoria?'))
            return;
        try {
            await api(`/finance/category-types/${id}`, {
                method: 'DELETE',
            });
    ...
    function renderGrid() {
        ...
        <div class="bg-white dark:bg-slate-800 shadow rounded-lg p-5 flex flex-col border border-gray-100 dark:border-slate-700 relative group transition-all duration-200 hover:shadow-md">
            ...
            <div class="flex-1 min-w-0 mb-4">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 line-clamp-2">${typeObj.description || 'Sem descrição'}</p>
            </div>
    ...
    function openModal(typeObj = null) {
        ...
        if (title)
            title.textContent = typeObj ? 'Editar Tipo de Categoria' : 'Cadastrar Tipo de Categoria';
        const nameInput = getEl('typeName');
        const descInput = getEl('typeDescription');
        const idInput = getEl('typeId');
        const form = getEl('typeForm');
        if (typeObj) {
            if (nameInput)
                nameInput.value = typeObj.name || '';
            if (descInput)
                descInput.value = typeObj.description || '';
            if (idInput)
    ...
    window.duplicateType = (publicId) => {
        const t = g_types.find((item) => item.public_id === publicId);
        if (t) {
            const dup = { ...t, public_id: '' };
            openModal(dup);
        }
    };
    // --- Utils ---
