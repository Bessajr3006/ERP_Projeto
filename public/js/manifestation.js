(() => {
    ...
    async function pollJobStatus(jobId, type) {
        const check = async () => {
            try {
                const response = await api(`/manifestation/status/${jobId}`);
    ...
    async function syncSefaz() {
        ...
        try {
            const response = await api(`/manifestation/consult-destined?lastNSU=${state.lastNSU}`);
    ...
    async function manifestNote(chNFe, tpEvento) {
        if (state.loading)
            return;
        const eventNames = {
        ...
        if (!confirm(`Deseja enviar o evento de "${eventNames[tpEvento]}" para a nota ${chNFe}?`))
            return;
        let xJust = '';
        if (tpEvento === '210240') {
        ...
        try {
            const response = await api('/manifestation/manifest', {
                method: 'POST',
    ...
    function showAlert(msg, type, isHtml = false) {
        const alert = document.getElementById('alertMessage');
        if (!alert)
            return;
        alert.classList.remove('hidden', 'bg-emerald-100', 'text-emerald-800', 'bg-red-100', 'text-red-800', 'bg-blue-100', 'text-blue-800');
        if (type === 'success')
            alert.classList.add('bg-emerald-100', 'text-emerald-800');
        else if (type === 'danger')
