(() => {
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        function showAlert(message, type = 'success', timeout = 5000) {
            ...
            els.alertMessage.className = `mx-4 sm:mx-0 mb-4 p-4 rounded-xl text-sm border font-medium ${type === 'error' ? 'bg-red-50 text-red-700 border-red-200' :
        ...
        async function loadSales() {
            ...
            try {
                ...
                if (res && res.status === 'success') {
                    ...
                    state.selectedBatchSaleIds.forEach(id => { if (eligibleIds.has(id.toString()))
                        newSelected.add(id); });
        ...
        function applyFilters() {
            ...
            if (qStatus === 'pending')
                res = res.filter(s => s.status !== 'invoiced');
            if (qStatus === 'invoiced')
                res = res.filter(s => s.status === 'invoiced');
            if (qSearch) {
                res = res.filter(s => String(s.id).includes(qSearch) ||
        ...
        function renderViewToggles() {
            ...
            els.viewModeBtns.forEach(btn => {
                ...
                btn.className = `view-btn flex items-center justify-center px-3 py-1.5 rounded-lg transition-all gap-1 ${isSelected ? 'bg-brand-100 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-200'}`;
        ...
        async function cancelNota(id) {
            if (!confirm('Deseja cancelar esta Nota Fiscal? O processo não pode ser revertido.'))
                return;
        ...
        async function transmitToSefaz(id) {
            if (!confirm('Deseja transmitir os dados da NFe para SEFAZ Estadual?'))
                return;
        ...
        async function generateAndShowXml(id) {
            ...
            try {
                ...
                if (!nfeResult || !nfeResult.xml)
                    throw new Error("XML não retornado.");
        ...
        function setupListeners() {
            ...
            els.notesContainer.addEventListener('click', (e) => {
                ...
                if (btnItems)
                    openItemsModal(btnItems.dataset.id);
                const btnEmitSingle = e.target.closest('.btn-emit-single');
                if (btnEmitSingle)
                    openEmitModal([btnEmitSingle.dataset.id]);
                const btnCancel = e.target.closest('.btn-cancel');
                if (btnCancel)
                    cancelNota(btnCancel.dataset.id);
                const btnTx = e.target.closest('.btn-transmit');
                if (btnTx)
                    transmitToSefaz(btnTx.dataset.id);
            ...
            els.notesContainer.addEventListener('change', (e) => {
                if (e.target.classList.contains('batch-sale-chk')) {
                    ...
                    if (e.target.checked)
                        state.selectedBatchSaleIds.add(id);
                    else
                        state.selectedBatchSaleIds.delete(id);
                    renderBatchToolbar();
                }
            });
            // Modals Closing
