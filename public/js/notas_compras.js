(() => {
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        function renderRows() {
            ...
            if (els.footerCount)
                els.footerCount.textContent = String(state.filteredPurchases.length);
            if (els.footerTotal)
                els.footerTotal.textContent = formatCurrency(totalAmount);
        }
        // --- Actions ---
        ...
        async function cancelNota(id) {
            if (!confirm('Deseja cancelar esta Nota Fiscal de Entrada?'))
                return;
