...
(() => {
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        function renderBoardSelect() {
            ...
            sel.innerHTML = data.boards.map(b => `<option value="${b.id}" ${b.id === currentBoardId ? 'selected' : ''}>${escHtml(b.name)}</option>`).join('');
        ...
        function renderBacklog() {
            ...
            const sectionsHtml = board.columns.map(col => {
                ...
                const rowsHtml = col.cards.map(card => {
                    ...
                    const assigneeHtml = (() => {
                        if (!card.assignee || card.assignee === 'all')
                            return dash;
        ...
        `;
            ...
            wrapper.querySelectorAll('.kanban-card').forEach(cardEl => {
                cardEl.addEventListener('click', e => {
                    if (e.target.closest('.btnCardEdit'))
        ...
        function onZoneDragOver(e) {
            ...
            if (!existing) {
                ...
                if (!afterEl)
                    ...
                    zone.insertBefore(ph, afterEl);
            }
            else {
                if (!afterEl)
                    zone.appendChild(existing);
                else
                    zone.insertBefore(existing, afterEl);
            }
        }
        ...
        function renderLabelPicker(cardId) {
            ...
            if (cardId) {
                const result = getCard(board, cardId);
                if (result)
                    activeLabels = result.card.labels || [];
            }
        ...
        function renderAssigneePicker(cardId) {
            ...
            sel.innerHTML = '<option value="all">Todos</option>' +
                usersData.filter(u => u.public_id).map(u => `<option value="${u.public_id}">${escHtml(u.full_name || u.email || u.public_id)}</option>`).join('');
            sel.value = current;
            // If the option doesn't exist yet (usersData empty), store for later
            if (sel.value !== current)
                sel.dataset.pending = current;
            else
        ...
        function renderHeaderColorPicker(cardId) {
            ...
            if (cardId) {
                const result = getCard(board, cardId);
                if (result)
                    activeColor = result.card.headerColor || '';
            }
            ...
            const colorHtml = HEADER_COLORS.map(hex => `<button type="button" class="hcolor-btn w-7 h-7 rounded-full shrink-0 transition-all" data-hcolor="${hex}" title="${hex}" style="background:${hex}" ${activeColor === hex ? 'data-selected' : ''}></button>`).join('');
        ...
        function saveCard() {
            ...
            if (!isNew) {
                ...
                const result = getCard(board, cardId);
                if (result)
                    Object.assign(result.card, cardData);
            }
            else {
                // new
            ...
            if (getNotifyToggle()) {
                ...
                if (assigneeId && assigneeId !== 'all')
                    msgParts.push(`Responsável: ${assigneeName}`);
                if (cardData.dueDate) {
        ...
        function onDeleteColumn(colId) {
            if (!confirm('Excluir esta lista e todos os seus cards?'))
                return;
            const board = getCurrentBoard();
        ...
        function renderColumnColorPicker(activeColorClass) {
            ...
            container.innerHTML = COLUMN_COLOR_CONFIG.map(({ cls, hex }) => `<button type="button" class="col-color-btn w-8 h-8 rounded-full shrink-0 transition-all" data-cls="${cls}" title="${cls}" style="background:${hex}" ${activeColorClass === cls ? 'data-selected' : ''}></button>`).join('');
        ...
        function confirmNameModal() {
            ...
            if (nameModalCallback)
                nameModalCallback(value, colorClass);
