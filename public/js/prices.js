(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        pricesManager = new window.CrudManager({
            ...
            renderTable: (items) => {
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold text-brand-600 dark:text-brand-400">${formatPerc(p.markup_percentage)}</td>
            ...
            renderGrid: (items) => {
                ...
                grid.innerHTML = items
                    .map((p) => `
                <div class="bg-white dark:bg-slate-800 shadow rounded-lg p-5 flex flex-col border ${p.status === 'active'
                    ? 'border-brand-500/30'
                    : 'border-gray-200 dark:border-slate-700 opacity-70'} relative group">
                    <div class="absolute top-2 right-2 flex space-x-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                        <button type="button" title="Editar" class="p-1.5 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:text-indigo-400 dark:hover:bg-indigo-900/30 rounded-md transition-colors edit-btn" data-item='${JSON.stringify(p).replace(/'/g, '&#39;')}'>
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                        </button>
                        <button type="button" title="Duplicar" class="p-1.5 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:text-indigo-400 dark:hover:bg-indigo-900/30 rounded-md transition-colors duplicate-btn" data-item='${JSON.stringify(p).replace(/'/g, '&#39;')}'>
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-100 dark:border-slate-700 flex justify-between items-center">
                        ...
                        <span class="text-xl font-black text-brand-600 dark:text-brand-400">${formatPerc(p.markup_percentage)}</span>
                    </div>
                </div>
            `)
                    .join('');
    ...
    getById('priceForm')?.addEventListener('submit', async (e) => {
        e.preventDefault();
        const saveBtn = getById('saveBtn');
        const form = getById('priceForm');
        const parseMarkup = (val) => parseFloat(String(val).replace(',', '.')) || 0;
        const payload = {
        ...
        try {
            if (form.dataset.id) {
                await api(`/estoque/prices/${form.dataset.id}`, {
                    method: 'PUT',
                    body: JSON.stringify(payload),
                });
                UI.showAlert('alertMessage', 'Tabela de preço atualizada com sucesso!', 'success');
            }
            else {
                await api('/estoque/prices', {
                    method: 'POST',
                    body: JSON.stringify(payload),
                });
                UI.showAlert('alertMessage', 'Tabela de preço salva com sucesso!', 'success');
            }
