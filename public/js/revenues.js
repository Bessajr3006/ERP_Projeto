...
(() => {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (customerSearch)
            customerSearch.addEventListener('input', syncCustomerSearch);
    ...
    function populateUserFilter() {
        ...
        revenuesData.forEach(row => {
            ...
            if (value)
                users.set(value, row.user_name || 'Usuario sem nome');
        });
        ...
        let filtered = revenuesData.filter(r => {
            ...
            if (status) {
                ...
                if (status === 'paid' && !isRevenuePaid(r))
                    match = false;
                if (status === 'pending' && (isRevenuePaid(r) || revenueStatus === 'progress'))
                    match = false;
            }
            if (bank) {
            ...
            if (user) {
                if (getRevenueUserFilterValue(r) !== user)
                    match = false;
            }
            return match;
        });
    ...
    function renderTable(data = revenuesData) {
        ...
        const translateDescription = (desc) => {
            if (!desc)
                return desc;
            return desc.replace(/\((cash|pix|credit|debit|transfer|boleto)\)/gi, (match) => paymentTermsPtBr[match.toLowerCase()] || match);
        };
    ...
    function openBaixaModal(rev) {
        ...
        if (descEl)
            descEl.textContent = rev.description || '-';
        if (amountEl)
            amountEl.textContent = formatCurrency(rev.amount || 0);
        if (customerEl)
                        ...
                        if (confirm('Esta receita ainda não possui boleto gerado no banco. Deseja emitir agora?')) {
                            ...
                            fetch('/api/v1/finance/revenues/' + pubId + '/generate-billet', {
                                ...
                                if (data.status === 'success') {
                                    window.UI.showAlert('alertMessage', 'Boleto gerado com sucesso!', 'success');
                                    fetchRevenues();
                                }
                                else {
                                    window.UI.showAlert('alertMessage', 'Erro: ' + data.message, 'error');
                                }
                            })
                                .catch(err => {
                ...
                if (pubId && confirm('Deseja realmente cancelar este boleto no banco? Esta ação não pode ser desfeita.')) {
                    ...
                    fetch('/api/v1/finance/revenues/batch-cancel-billets', {
                        ...
                        if (data.status === 'success') {
                            ...
                            window.UI.showAlert('alertMessage', 'Erro ao cancelar: ' + data.message, 'error');
                        }
                    })
                        .catch(err => {
                        window.UI.showAlert('alertMessage', 'Erro de conexão: ' + err.message, 'error');
    ...
    function openBoletoModal(pubId, nossoNumero) {
