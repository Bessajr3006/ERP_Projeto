    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        async function loadLinkableEntities() {
            try {
                ...
                console.error("Falha carregar entidades", e);
            }
        }
        // Resolutor de Nomes
                ...
                for (let h = 6; h <= 23; h++) {
                    ...
                    if (hourTasks.length > 0) {
                        tasksHtml = hourTasks.map(t => {
                            ...
                            if (isDone)
                                bgClass = 'bg-emerald-50 border border-emerald-200 text-emerald-800 dark:bg-emerald-900/40 dark:border-emerald-800 dark:text-emerald-300 opacity-60';
                            else if (dateStr < todayStr)
                                bgClass = 'bg-red-50 border border-red-200 text-red-800 dark:bg-red-900/40 dark:border-red-800 dark:text-red-300 hover:shadow-md cursor-pointer';
            ...
            `;
                ...
                for (let d = 1; d <= lastDate; d++) {
                    ...
                    let tasksListHtml = tasksForDay.map(t => {
                        ...
                        if (isDone)
                            bgClass = 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300 opacity-70';
            ...
            if (t.dueDate) {
                ...
                if (!isDone) {
                    if (datePart < todayStr)
                        colorClass = 'text-red-500 font-semibold'; // Atrasado
                    else if (datePart === todayStr)
                        colorClass = 'text-orange-500 font-semibold'; // Hoje
                    else
                        colorClass = 'text-brand-600 dark:text-brand-400'; // Futuro
                }
        ...
        function openTaskModal(titleText) {
            if (taskModalTitle)
                taskModalTitle.textContent = titleText || 'Nova Tarefa';
            taskModal.classList.remove('hidden');
            document.body.classList.add('overflow-hidden');
            setTimeout(() => { if (inputTitle)
                inputTitle.focus(); }, 50);
        }
        ...
        if (btnOpenTaskModal)
            btnOpenTaskModal.addEventListener('click', () => openTaskModal('Nova Tarefa'));
