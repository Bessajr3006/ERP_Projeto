(() => {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        let emailConfigLoadedOnce = false;
        ...
        function switchTab(tab) {
            ...
            // hide userForm save button if we're on the email tab, since it has its own form
            if (footer) footer.classList.toggle('hidden', tab === 'email');
            ...
            if (tab === 'email') {
                if (!emailConfigLoadedOnce) {
                    emailConfigLoadedOnce = true;
        ...
        async function saveEmailConfig(event) {
            ...
            try {
                ...
                if (btn) {
                    btn.disabled = false;
                    btn.innerHTML = `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Salvar E-mail`;
                }
            }
        }
        // --- Save ---
        ...
        getById('toggleUserSmtpPassword')?.addEventListener('click', (e) => {
            ...
            if (input) {
                if (input.type === 'password') {
                    input.type = 'text';
                    e.currentTarget.innerHTML = `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.29 3.29m0 0a10.05 10.05 0 011.563-3.029m5.858.908a3 3 0 014.243 4.243m-4.242 4.242l3.29 3.29M21 21l-3.29-3.29"/></svg>`;
                } else {
                    input.type = 'password';
