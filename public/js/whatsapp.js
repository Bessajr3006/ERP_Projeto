(() => {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        let currentPhoneAliases = [];
        let currentPreferredChatId = null;
        let currentName = null;
        let allConversations = [];
        let allCustomerContacts = [];
        let registeredContactRolesByPhone = new Map();
        ...
        function getActivityAgeMs() {
            return Date.now() - lastUserActivityAt;
        }
        function computeConversationsPollMs() {
        ...
        function computeMessagesPollMs() {
            const activityAge = getActivityAgeMs();
            if (!isChatVisible()) {
        ...
        function refreshAdaptivePolling() {
        ...
        function setSessionButtonsState(isLoading = false) {
        ...
        function focusMessageInputSafely() {
            if (!messageInput || !canAutofocus())
                return;
        ...
        let registerTypeMenuCloseHandler = null;
        ...
        function redirectToEntityRegistration(entityKey, page, contactName, contactPhone) {
        ...
        function showRegisterTypeMenu(trigger, contactName, contactPhone) {
            ...
            registerTypeMenuCloseHandler = (event) => {
        ...
        function shouldRenderMessageText(message) {
        ...
        function parseLocationCoordinate(value, min, max) {
        ...
        function parseLocationFromText(messageText) {
        ...
        function buildLocationHtml(message) {
        ...
        function tokenizeContactName(value) {
        ...
        function isLikelyTransientWhatsAppId(phone) {
            const digits = String(phone || '').replace(/\D/g, '');
            return /^\d{14,}$/.test(digits) && !digits.startsWith('55');
        }
        function getChatUserFromChatId(chatId) {
        ...
        function pickPreferredPhoneForSend(values) {
        ...
        function getMessageIdNumber(message) {
        ...
        function pickPreferredChatId(messages) {
        ...
        function pickBetterContactName(left, right) {
        ...
        function areLikelySamePersonByName(leftName, rightName) {
            const normalizedLeft = normalizeContactName(leftName);
            const normalizedRight = normalizeContactName(rightName);
            if (!normalizedLeft || !normalizedRight)
                return false;
        ...
        function getPhoneSuffix(value, size = 8) {
        ...
        function canMergeConversationEntries(existing, candidate) {
            ...
            const sameName = areLikelySamePersonByName(getConversationDisplayName(existing), getConversationDisplayName(candidate));
            if (!sameName)
                return false;
        ...
        function addRoleForPhone(rolesMap, phone, role) {
            if (!phone || !role)
                return;
        ...
        function getRegisteredRoleLabels(aliasPhones) {
        ...
        function getMergedConversationList() {
            ...
            for (const conversation of mergedList) {
                ...
                for (const existing of dedupedByName) {
                    ...
                    const existingIsTransient = isLikelyTransientWhatsAppId(existingPhone);
                    const candidateIsTransient = isLikelyTransientWhatsAppId(candidatePhone);
                    const aliasSet = new Set(uniquePhones([
                        ...(Array.isArray(existing?.alias_phones) ? existing.alias_phones : []),
                        existingPhone,
                        candidatePhone,
                    ]));
                    const preferredName = pickBetterContactName(getConversationDisplayName(existing), getConversationDisplayName(conversation));
        ...
        function renderConversations() {
            ...
            const filtered = searchTerm
                ? mergedConversations.filter(c => (c.contact_name || '').toLowerCase().includes(searchTerm) ||
        ...
        async function selectConversation(phone, name, phonesRaw, chatIdRaw) {
            ...
            if (chatPhone)
                chatPhone.textContent = currentPhone;
            if (chatAvatar) {
        ...
        async function loadMessages(scrollToBottom = false) {
            ...
            try {
                ...
                const responses = await Promise.all(phonesToLoad.map((phone) => api(`/users/${userId}/whatsapp-business/messages?phone=${encodeURIComponent(phone)}&limit=200`)));
                const mergedByPublicId = new Map();
                ...
                for (const response of responses) {
                    ...
                    for (const message of list) {
                        ...
                        if (mergedByPublicId.has(publicId))
                            continue;
        ...
        async function deleteCurrentConversation() {
            markUserActivity();
            if (!currentPhone || deletingConversation)
                return;
        ...
        async function sendMessage() {
            ...
            if (attachmentClearBtn)
                attachmentClearBtn.disabled = true;
            if (messageInput)
                messageInput.value = '';
            try {
                const payload = { to: targetPhone };
                ...
                if (typeof UI !== 'undefined' && UI.showAlert) {
                    try {
                        const errorMessage = e?.message || 'Falha ao enviar mensagem no WhatsApp.';
                ...
                if (attachmentClearBtn)
                    attachmentClearBtn.disabled = false;
        ...
        function startPolling() {
            ...
            const tick = async () => {
                if (document.hidden || convPollInFlight)
                    return;
        ...
        function startMsgPolling() {
            ...
            const tick = async () => {
                if (document.hidden || msgPollInFlight || !currentPhone || !isChatVisible())
                    return;
