...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const selectAllBackup = document.getElementById('selectAllBackup') as HTMLInputElement | null;
    ...
    const loadBackupTables = async () => {
        try {
            ...
            if (backupTablesContainer) {
                ...
                const checkBoxes = backupTablesContainer.querySelectorAll('.table-backup-checkbox') as NodeListOf<HTMLInputElement>;
    ...
    if (selectAllBackup) {
        selectAllBackup.addEventListener('change', () => {
            const checkBoxes = document.querySelectorAll('.table-backup-checkbox') as NodeListOf<HTMLInputElement>;
            checkBoxes.forEach(cb => {
    ...
    if (startBackupBtn) {
        startBackupBtn.addEventListener('click', async () => {
            ...
            const checkBoxes = document.querySelectorAll('.table-backup-checkbox') as NodeListOf<HTMLInputElement>;
            checkBoxes.forEach(cb => {
                if (cb.checked) selectedTables.push(cb.value);
            });
    ...
    const selectAllRestore = document.getElementById('selectAllRestore') as HTMLInputElement | null;
    ...
    const updateRestoreBtnState = () => {
        ...
        const checkedBoxes = document.querySelectorAll('.table-restore-checkbox:checked') as NodeListOf<HTMLInputElement>;
    ...
    if (restoreFileInput && startRestoreBtn) {
        restoreFileInput.addEventListener('change', async () => {
            if (restoreFileInput.files && restoreFileInput.files.length > 0) {
                ...
                if (restoreTablesSelectionSection) restoreTablesSelectionSection.classList.remove('hidden');
                ...
                try {
                    ...
                    if (restoreTablesContainer) {
                        ...
                        const checkBoxes = restoreTablesContainer.querySelectorAll('.table-restore-checkbox') as NodeListOf<HTMLInputElement>;
                        checkBoxes.forEach(cb => {
                            cb.addEventListener('change', () => {
                                if (selectAllRestore) {
                                    const allChecked = Array.from(checkBoxes).every(c => c.checked);
                ...
                if (restoreTablesSelectionSection) restoreTablesSelectionSection.classList.add('hidden');
                startRestoreBtn.disabled = true;
            }
        });
        ...
        if (selectAllRestore) {
            selectAllRestore.addEventListener('change', () => {
                const checkBoxes = document.querySelectorAll('.table-restore-checkbox') as NodeListOf<HTMLInputElement>;
                checkBoxes.forEach(cb => {
