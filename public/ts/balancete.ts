(() => {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    function getProcessedBalancete(): { rows: any[]; totals: { sumDb: number; sumCr: number } } {
      ...
      entries.forEach((entry: any) => {
        ...
        if (isBefore) {
          if (debitAcc) debitAcc.saldoAnteriorDb += amount;
          if (creditAcc) creditAcc.saldoAnteriorCr += amount;
        } else {
          if (debitAcc) debitAcc.debitosPeriodo += amount;
          if (creditAcc) creditAcc.creditosPeriodo += amount;
        }
      });
      ...
      const getDC = (val: number): string => (val > 0.001 ? '(D)' : val < -0.001 ? '(C)' : '');
      const getDCAbs = (val: number): string => (Math.abs(val) < 0.001 ? '' : getDC(val));
      ...
      sortedCodesAsc.forEach((code: string) => {
        ...
        if (filters.hideZeros && isZero) return;
        ...
        const displaySa =
          formatCurrency(Math.abs(saLiquido)) +
        ...
        const displaySf =
          formatCurrency(Math.abs(sfLiquido)) +
          ' <span class="text-gray-400 text-[10px] ml-1">' +
          getDCAbs(sfLiquido) +
          '</span>';
    ...
    function renderBalancete(): void {
      ...
      setTimeout(() => {
        ...
        if (rows.length === 0) {
          ...
          if (els.footerTotals) els.footerTotals.classList.add('hidden');
          if (els.loadingOverlay) {
        ...
        if (els.footerSumDb) els.footerSumDb.textContent = formatCurrency(totals.sumDb);
        if (els.footerSumCr) els.footerSumCr.textContent = formatCurrency(totals.sumCr);
