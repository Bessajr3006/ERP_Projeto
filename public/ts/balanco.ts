  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    const generateBalanco = (): void => {
      ...
      entries.forEach((entry: any) => {
        ...
        if (debitAcc && debitAcc.code?.startsWith('1')) debitAcc.balance += amount;
        if (creditAcc && creditAcc.code?.startsWith('1')) creditAcc.balance -= amount;

        if (creditAcc && creditAcc.code?.startsWith('2')) creditAcc.balance += amount;
        if (debitAcc && debitAcc.code?.startsWith('2')) debitAcc.balance -= amount;
      });
      ...
      codesToRender.forEach((code) => {
        ...
        if (hideZero && Math.abs(rawBalance) < 0.001) return;
        if (onlySynthetic && !isSynthetic) return;
        ...
        let cssClass = '';
        if (code.startsWith('1')) cssClass = 'text-blue-600 dark:text-blue-400';
        else cssClass = 'text-indigo-600 dark:text-indigo-400';

        const displayValue = showValue ? Formatters.formatCurrency(rawBalance) : '';

        let av = 0;
        if (code.startsWith('1') && totalAtivo > 0) av = (rawBalance / totalAtivo) * 100;
        if (code.startsWith('2') && totalPassivo > 0) av = (rawBalance / totalPassivo) * 100;
        ...
        rows.push(`
          <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-700/50 transition-colors">
              <td class="px-3 py-2 text-xs text-gray-900 dark:text-gray-100 ${
                isSynthetic ? 'font-bold' : ''
              }" style="padding-left: ${0.75 + indent}rem">
                  ${acc.code} - ${acc.name}
              </td>
              <td class="px-3 py-2 text-right text-xs ${
                isSynthetic ? 'font-bold' : 'font-medium'
              } ${cssClass}">
                  ${displayValue}
              </td>
      ...
      if (rows.length === 0) {
        ...
        if (balancoTableFooter) balancoTableFooter.className = 'hidden';
        return;
      }

      if (balancoTableBody) balancoTableBody.innerHTML = rows.join('');
      ...
      if (Math.abs(resultadoExercicio) > 0.01) {
        ...
        diffAlertHtml = `
          <div class="mt-4 p-3 bg-${isLucro ? 'green' : 'orange'}-100 dark:bg-${
            isLucro ? 'green' : 'orange'
          }-900/30 rounded-lg flex flex-wrap items-center justify-between">
              <span class="text-xs font-semibold text-${isLucro ? 'green' : 'orange'}-800 dark:text-${
          isLucro ? 'green' : 'orange'
        }-300">
                  Resultado Acumulado (DRE) não encerrado no Passivo
              </span>
              <span class="text-sm font-bold text-${isLucro ? 'green' : 'orange'}-600 dark:text-${
          isLucro ? 'green' : 'orange'
        }-400">
              ...
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div class="flex gap-4 sm:gap-8 items-center justify-end w-full sm:w-auto">
                      <div class="text-right">
                          ...
                          <span class="block text-xl font-black text-blue-600 dark:text-blue-400">${Formatters.formatCurrency(
                            totalAtivo
                          )}</span>
                      </div>
                      <div class="text-right pr-2">
                          <span class="block text-[10px] font-bold uppercase tracking-wider text-gray-500">Passivo + PL</span>
                          <span class="block text-xl font-black text-indigo-600 dark:text-indigo-400">${Formatters.formatCurrency(
                            totalPassivoEB
                          )}</span>
                      </div>
                  </div>
