(() => {
  ...
  const setupFileDropzones = (): void => {
    const setupZone = (
      dropzoneId: string,
      fileInputId: string,
      textareaId: string,
      filenameDivId: string,
      filenameTextId: string,
      clearBtnId: string
    ) => {
      ...
      ['dragenter', 'dragover'].forEach((eventName) => {
        dropzone.addEventListener(
          eventName,
          () => dropzone.classList.add('border-brand-500', 'bg-brand-50', 'dark:bg-slate-700/50'),
          false
        );
      });
      ...
      ['dragleave', 'drop'].forEach((eventName) => {
        dropzone.addEventListener(
          eventName,
          () => dropzone.classList.remove('border-brand-500', 'bg-brand-50', 'dark:bg-slate-700/50'),
          false
        );
      });
  ...
  const populateFileDropzoneUI = (type: string, hasContent: boolean): void => {
    const dropzone = getById(`dropzone${type}`);
    const filenameDiv = getById(`filename${type}`);
  ...
  const renderTable = (elementId: string, items: any[]): void => {
    ...
    const typeLabels: Record<string, string> = {
      checking:
        '<span class="px-2 py-1 bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 text-xs rounded-md font-medium">Conta Corrente</span>',
      savings:
        '<span class="px-2 py-1 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 text-xs rounded-md font-medium">Poupança</span>',
      cash:
        '<span class="px-2 py-1 bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 text-xs rounded-md font-medium">Caixa Físico</span>',
    };
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                ...
                <div class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    <span class="font-mono text-[10px] select-all">${b.public_id}</span>
                    <button type="button" class="view-id-btn text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 transform transition-all duration-200 ease-out" data-id="${b.public_id}" title="Copiar ID: ${b.public_id}">
    ...
    if (selectAllBtn) {
      ...
      newSelectAll.addEventListener('change', (e: any) => {
  ...
  const renderGrid = (elementId: string, items: any[]): void => {
    ...
    const typeLabels: Record<string, string> = {
      checking:
        '<span class="px-2 py-0.5 bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 text-xs rounded-md font-medium">Conta Corrente</span>',
      savings:
        '<span class="px-2 py-0.5 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 text-xs rounded-md font-medium">Poupança</span>',
      cash:
        '<span class="px-2 py-0.5 bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 text-xs rounded-md font-medium">Caixa Físico</span>',
    };
  ...
  const bindActionEvents = (): void => {
    ...
    document.querySelectorAll('.duplicate-btn').forEach((btn: any) => {
      btn.addEventListener('click', (e: any) => {
        const id = e.currentTarget.getAttribute('data-id');
        duplicateBank(id);
      });
  ...
  const applyFilters = (): void => {
    ...
    const filtered = _banksData.filter((item: any) => {
  ...
  const editBank = (id: string): void => {
    const bank = _banksData.find((b: any) => b.public_id === id);
    if (!bank) return;
  ...
  const duplicateBank = (id: string): void => {
    const bank = _banksData.find((b: any) => b.public_id === id);
    if (!bank) return;
  ...
  (window as any).setupFileDropzones = setupFileDropzones;
  (window as any).resetFileDropzonesUI = resetFileDropzonesUI;
  (window as any).populateFileDropzoneUI = populateFileDropzoneUI;
  (window as any).openModal = openModal;
  (window as any).closeModal = closeModal;
  (window as any).editBank = editBank;
  (window as any).duplicateBank = duplicateBank;
  (window as any).deleteBank = deleteBank;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const initialBalanceInputEl = getById('initialBalance');
    initialBalanceInputEl?.addEventListener('input', (e: any) => {
  ...
  getById('bankForm')?.addEventListener('submit', async (e: any) => {
    e.preventDefault();
