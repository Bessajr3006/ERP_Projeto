...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const company = userInfo.data.company;
            ...
            const publicIdEl = document.getElementById('companyPublicId') as HTMLInputElement | null;
            if (publicIdEl) publicIdEl.value = company.public_id || '';
            ...
            if (taxRegimeEl) taxRegimeEl.value = company.tax_regime || '';
            const companyActiveEl = document.getElementById('companyActive');
            if (companyActiveEl) companyActiveEl.checked = company.is_active !== false;
            const allowPrintEl = document.getElementById('allowPrintWithoutConfirmation');
            if (allowPrintEl) allowPrintEl.checked = !!company.allow_print_without_confirmation;
            const statePreviewEl = document.getElementById('parameterStatePreview');
            if (statePreviewEl) statePreviewEl.value = company.state || 'Não definido';
            ...
            if (docMask && company.cnpj) {
                ...
                if (hiddenUser) hiddenUser.value = company.cnpj;
            }
