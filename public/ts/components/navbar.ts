...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!navbarContainer) return; // Página não precisa de navbar
    ...
    try {
        if (typeof api !== 'undefined') {
            ...
            if (data && data.data && data.data.user && data.data.user.full_name) {
                ...
                if ((role === 'admin' || role === 'super_admin') && permissions.length === 0) return;
                ...
                const ROLE_FALLBACK_MODULES: Record<string, string[]> = {
                ...
                allLinks.forEach(a => {
                    ...
                    if (a.dataset.noPermCheck === 'true') return;
    ...
    const pollBackgroundTasks = async () => {
        try {
            ...
            if (result?.status === 'success' && Array.isArray(result.data)) {
                ...
                if (lastNotifiedTransactions.length > 50) lastNotifiedTransactions = lastNotifiedTransactions.slice(-50);
...
async function initUserMenuChangePassword() {
    ...
    const form = document.getElementById('navChangePasswordForm') as HTMLFormElement;
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            const currentPass = (document.getElementById('navCurrentPassword') as HTMLInputElement).value;
            const newPass = (document.getElementById('navNewPassword') as HTMLInputElement).value;
            const confirmPass = (document.getElementById('navConfirmNewPassword') as HTMLInputElement).value;
            ...
            if (saveText) saveText.textContent = 'Alterando...';
            ...
            try {
                ...
                if (saveText) saveText.textContent = 'Alterar Senha';
            }
        });
    }
}
