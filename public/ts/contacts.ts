...
let contactsManager: any;
let contactsDocMask: any = null;
let contactsPhoneMask: any = null;
let contactsZipMask: any = null;
let contactsIbgeStates: Array<{ uf?: string; name?: string }> = [];
...
function formatContactLocation(item: any) {
...
function populateContactStateOptions(selectedValue = '') {
    ...
    if (!stateSelect || !contactsIbgeStates.length) return;
...
async function loadDependencies(selectedState = '') {
    try {
        ...
        if (!contactsIbgeStates.length) contactsIbgeStates = statesRes.data || [];
...
function applyContactCepLookupResult(data: any) {
    if (!data) return;
...
function applyContactPrefillFromQuery() {
    ...
    if (params.get('prefill') !== 'contacts') return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    contactsManager = new CrudManager({
        ...
        applyFilters: (data: any[]) => {
            const search = (window as any).FilterPanel.normalizeText(getById('filterSearch')?.value);
            const searchDigits = (window as any).FilterPanel.onlyDigits(search);

            const filtered = data.filter((item: any) => {
                if (!search) return true;

                if ((window as any).FilterPanel.matchesSearch(item, ['name', 'email', 'cnpj_cpf', 'phone', 'city', 'state'], search)) return true;
                if (!searchDigits) return false;

                return [item.cnpj_cpf, item.phone]
                    .map((value) => (window as any).FilterPanel.onlyDigits(value))
