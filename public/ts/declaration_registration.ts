(function initDeclarationRegistrationPage() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        declarationManager = new (window as any).CrudManager({
            ...
            renderGrid: (items: any[]) => {
                ...
                grid.innerHTML = items.map((d: any) => `
    ...
    // Form Logic
    getById('declarationTypeForm').addEventListener('submit', async (e: Event) => {
        e.preventDefault();
        ...
        try {
            if (form.dataset.id) {
                await (window as any).api(`/accounting/declaration-types/${form.dataset.id}`, {
                    method: 'PUT',
                    body: JSON.stringify(payload)
                });
                (window as any).UI.showAlert('alertMessage', 'Declaração atualizada com sucesso!', 'success');
            } else {
                await (window as any).api('/accounting/declaration-types', {
                    method: 'POST',
                    body: JSON.stringify(payload)
                });
                (window as any).UI.showAlert('alertMessage', 'Declaração salva com sucesso!', 'success');
            }
