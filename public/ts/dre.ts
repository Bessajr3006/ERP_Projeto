(() => {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (els.filterStartDate) els.filterStartDate.value = filters.startDate;
    if (els.filterEndDate) els.filterEndDate.value = filters.endDate;
    if (els.filterHideZeros) els.filterHideZeros.checked = filters.hideZeros;
    if (els.filterOnlySynthetic) els.filterOnlySynthetic.checked = filters.onlySynthetic;
    ...
    // --- Init ---
    if (typeof Auth !== 'undefined' && !(Auth as any).isAuthenticated()) {
    ...
    async function loadData(): Promise<void> {
      ...
      try {
        const [accRes, entRes] = await Promise.all([
          (api as any)('/accounting/chart-of-accounts'),
          (api as any)('/accounting/entries'),
        ]);
    ...
    function getProcessedDRE(): {
      rows: any[];
      totals: { totalReceitasBrutas: number; resultadoFinal: number; isProfit: boolean };
    } {
      let filteredEntries: any[] = entries;
      if (filters.startDate) {
        filteredEntries = filteredEntries.filter(
          (e: any) => String(e?.entry_date || '').substring(0, 10) >= filters.startDate
        );
      }
      if (filters.endDate) {
        filteredEntries = filteredEntries.filter(
          (e: any) => String(e?.entry_date || '').substring(0, 10) <= filters.endDate
        );
      }
      ...
      filteredEntries.forEach((entry: any) => {
        ...
        if (accountMap[entry.debit_account_code]) accountMap[entry.debit_account_code].balance += amount;
        if (accountMap[entry.credit_account_code]) accountMap[entry.credit_account_code].balance -= amount;
      });

      // Rollup
      const sortedCodesDesc = Object.keys(accountMap).sort().reverse();
      sortedCodesDesc.forEach((code: string) => {
        ...
        if (lastSep > 0) parentCode = code.substring(0, lastSep);
        ...
        while (parentCode && !accountMap[parentCode] && (parentCode.includes('.') || parentCode.includes('-'))) {
          ...
          if (nextSep > 0) parentCode = parentCode.substring(0, nextSep);
      ...
      sortedCodesAsc.forEach((code: string) => {
        const acc = accountMap[code];
        const rawBalance = acc.balance + acc.children_total;
        const isSynthetic = acc.type === 'synthetic';

        if (filters.hideZeros && rawBalance === 0) return;
        if (filters.onlySynthetic && !isSynthetic) return;
        ...
        if (displayValue === 0) cssClass = 'text-gray-900 dark:text-gray-100';
    ...
    function renderDRE(): void {
      ...
      if (els.tableBody) els.tableBody.innerHTML = html;
      ...
      const avTotal =
        totals.totalReceitasBrutas > 0
          ? ((Math.abs(totals.resultadoFinal) / totals.totalReceitasBrutas) * 100).toFixed(1) + '%'
          : '0.0%';
      if (els.footerAv) els.footerAv.textContent = avTotal;
    }

    // --- Listeners Setup ---
    function setupListeners(): void {
      els.toggleFilterBtn?.addEventListener('click', () => {
        if (!els.filterBody || !els.filterChevron) return;
      ...
      els.filterForm?.addEventListener('submit', (e: any) => {
        e.preventDefault();
      ...
      els.btnClearFilters?.addEventListener('click', () => {
        ...
        if (els.filterHideZeros) els.filterHideZeros.checked = false;
        if (els.filterOnlySynthetic) els.filterOnlySynthetic.checked = false;
