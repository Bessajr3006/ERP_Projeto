(() => {
  ...
  let currentView: 'list' | 'grid' = ((localStorage.getItem('expensesView') as any) || 'list') === 'grid' ? 'grid' : 'list';
  ...
  async function loadDependencies(): Promise<void> {
    try {
      ...
      if (bankSelect) bankSelect.innerHTML = '<option value="">Selecione a conta...</option>';
    ...
    tbody.innerHTML =
      items
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
            ...
            <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 hidden md:table-cell">${
              e.bank_account_name || '-'
            }</td>
