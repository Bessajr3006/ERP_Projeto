...
function renderTable(elementId, items) {
    ...
    if (manufacturersManager) manufacturersManager._bindActionEvents();
}
...
function setImagePreview(src: string | null): void {
    const preview = document.getElementById('manufacturerImgPreview') as HTMLImageElement | null;
    ...
    const hidden = document.getElementById('manufacturerImageBase64') as HTMLInputElement | null;
    const fileInput = document.getElementById('manufacturerImgInput') as HTMLInputElement | null;
    if (!preview || !placeholder || !clearBtn || !hidden) return;
    if (src) {
        hidden.value = src;
        preview.src = src;
...
function initImageUpload(): void {
    ...
    fileInput.addEventListener('change', () => {
        ...
        reader.onload = (e) => setImagePreview((e.target as FileReader).result as string);
