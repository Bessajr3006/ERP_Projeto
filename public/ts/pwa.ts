...
window.addEventListener('beforeinstallprompt', (e: BeforeInstallPromptEvent) => {
  const installButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('.pwaInstallBtn'));
...
function ensureMetaTag(name: string, content = ''): HTMLMetaElement {
...
function startInstallButtonsObserver() {
  if (installButtonsObserverStarted || !document.body) return;
...
function refreshInstallButtons() {
  const installBtns = document.querySelectorAll<HTMLButtonElement>('.pwaInstallBtn');
...
function bindInstallButton(btn: HTMLButtonElement, mode: string, handler: () => void) {
  if (btn.dataset.pwaMode === mode) return;
...
function createMaskAdapter(input: HTMLInputElement | null, options: { mask?: any } = {}) {
