(() => {
  ...
  let g_types: ReceivableType[] = [];
  let g_banks: BankAccount[] = [];
  let g_filteredTypes: ReceivableType[] = [];
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    getEl<HTMLFormElement>('receivableTypeForm')?.addEventListener('submit', handleSaveType);
  ...
  async function fetchBanks(): Promise<void> {
    try {
      const res = await (api as any)('/bank-accounts');
      g_banks = res.data || [];

      const select = getEl<HTMLSelectElement>('receivableTypeBankAccount');
      if (select) {
  ...
  async function fetchTypes(): Promise<void> {
    try {
      const res = await (api as any)('/receivable-types');
      g_types = res.data || [];
      applyFilters();
    } catch (error) {
  ...
  async function handleSaveType(e: Event): Promise<void> {
    ...
    const name = getEl<HTMLInputElement>('receivableTypeName')?.value;
    const bankAccountIdStr = getEl<HTMLSelectElement>('receivableTypeBankAccount')?.value;
    ...
    try {
      if (g_editingId) {
        await (api as any)(`/receivable-types/${g_editingId}`, {
          method: 'PUT',
          body: JSON.stringify(data),
        });
        showAlert('Tipo de recebível atualizado com sucesso!', 'success');
      } else {
        await (api as any)('/receivable-types', {
          method: 'POST',
          body: JSON.stringify(data),
        });
  ...
  (window as any).deleteType = async (id: string) => {
    ...
    try {
      await (api as any)(`/receivable-types/${id}`, {
        method: 'DELETE',
      });
  ...
  function openModal(typeObj: ReceivableType | null = null): void {
    g_editingId = typeObj ? typeObj.public_id || null : null;

    const title = getEl('modalTitle');
    if (title) title.textContent = typeObj ? 'Editar Tipo de Recebível' : 'Cadastrar Tipo de Recebível';

    const nameInput = getEl<HTMLInputElement>('receivableTypeName');
    const bankSelect = getEl<HTMLSelectElement>('receivableTypeBankAccount');
    const idInput = getEl<HTMLInputElement>('receivableTypeId');
    const form = getEl<HTMLFormElement>('receivableTypeForm');

    if (typeObj) {
      if (nameInput) nameInput.value = typeObj.name || '';
      if (bankSelect) bankSelect.value = String(typeObj.bank_account_id || '');
  ...
  function closeModal(): void {
    ...
    getEl<HTMLFormElement>('receivableTypeForm')?.reset();
    g_editingId = null;
  }
