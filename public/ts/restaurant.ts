(() => {
  ...
  let orderNumber = '';
  let observation = '';
  let openComandas: string[] = []; // Array of open order numbers
  ...
  function renderCategories(): void {
    ...
    categories.forEach((cat) => {
      ...
      btn.className = `cat-btn w-full bg-white dark:bg-slate-800 border ${
        isActive
          ? 'border-brand-500 ring-2 ring-brand-500/20'
          : 'border-gray-200 dark:border-slate-700'
      } shadow-sm rounded-xl p-4 flex flex-col items-center justify-center gap-3 hover:border-brand-400 hover:shadow-md transition-all group`;
      (btn as any).dataset.cat = cat;
      ...
      btn.addEventListener('click', () => {
        // Update UI state
        document.querySelectorAll<HTMLElement>('.cat-btn').forEach((b) => {
        ...
        if (currentCategoryTitle) currentCategoryTitle.textContent = cat;
  ...
  function renderProducts(searchQuery = ''): void {
    ...
    if (searchQuery) {
      ...
      if (currentCategoryTitle) currentCategoryTitle.textContent = `Busca: "${searchQuery}"`;
    } else {
      if (currentCategory) {
    ...
    filtered.forEach((p) => {
      const price = Number((p.price as any) || (p.sale_price as any) || 0)
        .toFixed(2)
        .replace('.', ',');
      ...
      card.innerHTML = `
            ...
            <div class="p-3 sm:p-4 flex flex-col flex-1 h-full min-w-0 justify-between">
                <div>
                    ...
                    <p class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 line-clamp-1 sm:line-clamp-2">${
                      p.description || ''
                    }</p>
                </div>
  ...
  function addToCart(product: Product): void {
    ...
    if (existing) {
      ...
      cart.push({
        id: product.id,
        name: product.name,
        price: Number((product.price as any) || (product.sale_price as any) || 0),
        qtd: 1,
        obs: '',
      });
    }
  ...
  function updateCartItemQtd(id: number, delta: number): void {
  ...
  (window as any).updateCartItemQtd = updateCartItemQtd;
  ...
  function updateCartUI(): void {
    ...
    const btnCheckout = document.getElementById('checkoutBtn') as HTMLButtonElement | null;

    // Badges
    const totalItems = cart.reduce((acc, i) => acc + i.qtd, 0);

    const mobileCartCount = document.getElementById('mobileCartCount');
    if (mobileCartCount) mobileCartCount.textContent = String(totalItems);

    const cartBadge = document.getElementById('cartBadge');
    if (cartBadge) cartBadge.textContent = `${totalItems} itens`;

    const cartSideBadge = document.getElementById('cartSideBadge');
    if (cartSideBadge) cartSideBadge.textContent = `${totalItems} itens`;

    // Clear Container except empty state
    if (container) {
      Array.from(container.children).forEach((child) => {
        if ((child as HTMLElement).id !== 'emptyCartState') {
          child.remove();
        }
      });
    }
      ...
      el.innerHTML = `
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center border border-gray-200 dark:border-slate-600 rounded-lg overflow-hidden bg-white dark:bg-slate-800">
                    ...
                    <span class="px-3 py-1 font-medium text-sm text-gray-700 dark:text-gray-300 border-x border-gray-200 dark:border-slate-600">${
                      item.qtd
                    }</span>
  ...
  function setupTabs(): void {
    ...
    const activeTabClass =
      'pos-tab-btn min-w-0 flex-1 md:flex-none truncate whitespace-nowrap px-3 sm:px-4 py-1.5 text-sm font-medium rounded-md bg-white dark:bg-slate-800 text-brand-600 dark:text-brand-400 shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-brand-500';
    const inactiveTabClass =
      'pos-tab-btn min-w-0 flex-1 md:flex-none truncate whitespace-nowrap px-3 sm:px-4 py-1.5 text-sm font-medium rounded-md text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-all focus:outline-none focus:ring-2 focus:ring-brand-500';

    if (btnProdutos && btnComandas && contentProdutos && contentComandas) {
      btnProdutos.addEventListener('click', () => {
        // Ativa aba de Produtos
        (btnProdutos as HTMLElement).className = activeTabClass;
        (btnComandas as HTMLElement).className = inactiveTabClass;
      ...
      btnComandas.addEventListener('click', () => {
        ...
        (btnComandas as HTMLElement).className = activeTabClass;
        (btnProdutos as HTMLElement).className = inactiveTabClass;
  ...
  function setupCategoriesNav(): void {
      ...
      if (removeBtn) {
        removeBtn.addEventListener('click', (e) => {
          ...
          if (confirm(`Tem certeza que deseja fechar a Mesa/Comanda #${comanda}?`)) {
            ...
            const numEl = document.getElementById('orderNumberInput') as HTMLInputElement | null;
  ...
  function setupComandaSearch(): void {
  ...
  function setupCartActions(): void {
  ...
  function setupOrderObs(): void {
    ...
    const input = document.getElementById('orderObs') as HTMLTextAreaElement | null;
