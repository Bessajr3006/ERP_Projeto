(() => {
  ...
  type State = {
    ...
    paymentMethodsList: { id: PaymentMethodId; name: string }[];
  };
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    const getCartTotal = (): number => Math.max(0, getCartSubtotal() - state.discount);

    const getTotalPaid = (): number => Object.values(state.payments).reduce((sum, val) => sum + (Number(val) || 0), 0);

    const getMissingAmount = (): number => getCartTotal() - getTotalPaid();
                          ...
                          <ul class="flex whitespace-nowrap -mb-px px-1 text-[12px] md:text-[13px] font-bold text-center gap-5">
                                ...
                                ${state.categories
                                  .map(
                                    (c) => `
                                    <li class="shrink-0">
                                        <button type="button" data-cat="${c.id}" class="cat-btn inline-block pb-3 px-1.5 border-b-2 transition-colors ${
                  ...
                  state.isCartOpen
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-700">
                        <div class="flex justify-between items-center mb-3">
                            <h1 class="text-[22px] font-bold text-gray-900 dark:text-gray-100 tracking-tight flex items-center gap-2">
                                ...
                                Venda <span class="text-[10px] bg-brand-600 text-white px-2 py-0.5 rounded shadow-sm">Cx. ${
                                  state.registerId
                                }</span>
                            </h1>
                            <div class="flex items-center gap-2 lg:gap-3">
                                <span class="text-[14px] font-normal text-gray-400 hidden md:block">${
                                  state.currentDate
                                }</span>
                    ...
                    <div class="px-6 pt-5 pb-6 bg-white dark:bg-slate-800 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] border-t border-gray-100 dark:border-slate-700">
                        <div class="flex justify-between items-center text-gray-600 mb-2">
                            <span class="text-[15px] font-medium">Subtotal</span><span class="font-semibold">${formatCurrency(
                              getCartSubtotal()
                            )}</span>
                        </div>
                        <div class="flex justify-between items-center text-gray-600 mb-4 pb-4 border-b border-gray-100">
                            ...
                            <span id="labelDiscount" class="font-semibold text-red-500 cursor-pointer">${formatCurrency(
                              state.discount
                            )}</span>
                        </div>
                        <div class="flex justify-between items-end mb-6">
                            <span class="text-[22px] font-black text-gray-900 dark:text-white tracking-tight">TOTAL</span>
                            <span class="text-[32px] font-black text-gray-900 dark:text-white tracking-tight">${formatCurrency(
                              getCartTotal()
                            )}</span>
                        </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl w-[95%] max-w-lg max-h-[90vh] shadow-2xl flex flex-col">
                    ...
                    <div class="p-6 space-y-6 flex-1 overflow-y-auto">
                        <div class="space-y-4">
                            ...
                            <select id="selectCustomer" class="w-full border p-2 rounded-lg bg-gray-50 dark:bg-slate-900 dark:border-slate-700 dark:text-white">
                                ...
                                ${state.customers
                                  .map(
                                    (c) =>
                                      `<option value="${c.public_id}" ${
                                        state.saleData.customerId === c.public_id ? 'selected' : ''
                                      }>${c.trade_name || c.name || ''}</option>`
                                  )
                                  .join('')}
                            </select>
                            <textarea id="deliveryAddress" rows="2" placeholder="Endereço de Entrega (Opcional)" class="w-full border p-2 rounded-lg bg-gray-50 dark:bg-slate-900 dark:border-slate-700 dark:text-white">${
                              state.saleData.deliveryAddress
                            }</textarea>
                        </div>
                        <div class="text-center p-4 bg-brand-50 dark:bg-brand-900/20 rounded-xl border border-brand-100 dark:border-brand-900/30">
                            ...
                            <p class="text-4xl font-black text-brand-800 dark:text-white">${formatCurrency(
                              getCartTotal()
                            )}</p>
                        </div>
                        <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="p-4 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30 flex flex-col justify-center ${
                              getMissingAmount() <= 0 ? 'opacity-50' : ''
                            }">
                                <p class="text-sm font-semibold text-red-600 uppercase">Falta Pagar</p>
                                <p class="text-2xl font-black text-red-700 dark:text-red-400">${formatCurrency(
                                  Math.max(0, getMissingAmount())
                                )}</p>
                            </div>
                            <div class="p-4 bg-gray-50 dark:bg-slate-900/50 rounded-xl border dark:border-slate-700 flex flex-col justify-center ${
                              getMissingAmount() > 0 ? 'opacity-50' : ''
                            }">
                                <p class="text-sm font-semibold text-gray-500 uppercase">Troco</p>
                                <p class="text-2xl font-black dark:text-white">${formatCurrency(
                                  Math.max(0, -getMissingAmount())
                                )}</p>
                            </div>
                        </div>
                ...
                ${filtered
                  .map(
                    ...
                    <div data-id="${p.public_id}" class="product-card bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200/60 dark:border-slate-700 overflow-hidden cursor-pointer hover:shadow-md hover:border-brand-300 transition-all group p-3 sm:p-4 flex flex-col h-full">
                        ...
                        <h4 class="font-medium text-[14px] sm:text-[16px] text-gray-800 dark:text-gray-200 leading-tight mb-2 line-clamp-2 mt-auto">${
                          p.name
                        }</h4>
                        <div class="flex flex-col justify-end mt-1">
                            <span class="text-[16px] sm:text-[18px] font-bold text-gray-900 dark:text-white">${formatCurrency(
                              getProductPrice(p)
                            )}</span>
                            ${
                              p.ean
                                ? `<div class="mt-1 flex items-center gap-1 text-[11px] text-gray-400 font-mono"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a2 2 0 002 2z"></path></svg>${
                                    p.ean
                                  }</div>`
                                : ''
                            }
          ...
          (item, idx) => `
            <div class="bg-white dark:bg-slate-800 rounded-xl p-3 shadow-sm border border-gray-100 dark:border-slate-700 relative group flex gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <h4 class="text-[13px] font-bold text-gray-800 dark:text-gray-200 line-clamp-2 leading-tight pr-5">${
                      item.product.name
                    }</h4>
                    <div class="flex justify-between items-center mt-2">
                        <span class="font-bold text-gray-900 dark:text-white text-[14px]">${formatCurrency(
                        ...
                        <div class="flex items-center bg-gray-50 dark:bg-slate-900 rounded-lg p-0.5 border border-gray-100 dark:border-slate-700">
                            <button type="button" class="btn-qty-minus w-7 h-7 flex items-center justify-center rounded bg-white dark:bg-slate-800 text-gray-500 hover:text-red-500 shadow-sm" data-idx="${
                              idx
                            }">-</button>
                            <span class="w-8 text-center text-xs font-bold dark:text-white">${item.quantity}</span>
                            <button type="button" class="btn-qty-plus w-7 h-7 flex items-center justify-center rounded bg-white dark:bg-slate-800 text-gray-500 hover:text-green-500 shadow-sm" data-idx="${
                              idx
                            }">+</button>
                        </div>
                    </div>
    ...
    function attachEventListeners(): void {
      ...
      (document.getElementById('selectCustomer') as HTMLSelectElement | null)?.addEventListener('change', (e: Event) => {
        state.saleData.customerId = (e.target as HTMLSelectElement).value;
      });

      (document.getElementById('deliveryAddress') as HTMLTextAreaElement | null)?.addEventListener('input', (e: Event) => {
        state.saleData.deliveryAddress = (e.target as HTMLTextAreaElement).value;
      });

      document.querySelectorAll('.payment-input').forEach((input) => {
        input.addEventListener('input', (e: Event) => {
          const el = e.target as HTMLInputElement;
          const method = el.dataset.method as PaymentMethodId | undefined;
          if (!method) return;
      ...
      document.querySelectorAll('.btn-fast-cash').forEach((btn) => {
        btn.addEventListener('click', (e: Event) => {
          const valStr = (e.currentTarget as HTMLElement | null)?.dataset?.val || '0';
    ...
    function updateQty(idx: number, delta: number): void {
      ...
      if (state.discount > getCartSubtotal()) state.discount = 0;
      render();
    }
    ...
    function askDiscount(): void {
      if (state.cart.length === 0) return void alert('Adicione produtos antes de aplicar desconto.');
      const val = prompt('Digite o valor do desconto (Ex: 10.50):');
      if (val !== null) {
        const num = parseFloat(val.replace(',', '.'));
        if (!isNaN(num) && num >= 0) {
          if (num > getCartSubtotal()) alert('Desconto inválido!');
          else {
            state.discount = num;
            render();
          }
    ...
    function openPaymentModal(): void {
      if (state.cart.length === 0) return void alert('Carrinho vazio.');
    ...
    try {
      ...
      if (bRes.data?.length > 0) state.defaultBankPublicId = bRes.data[0].public_id;
