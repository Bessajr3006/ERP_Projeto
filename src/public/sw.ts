...
const CACHE_NAME = 'keystone-pwa-v20260610004853';
...
function normalizeCacheKey(requestOrUrl: Request | string): string | null {
...
async function matchCachedResponse(request: Request): Promise<Response | undefined> {
  const cacheKey = normalizeCacheKey(request);
  if (!cacheKey) return undefined;
  return caches.match(cacheKey);
}

async function storeCachedResponse(request: Request, response: Response): Promise<void> {
  const cacheKey = normalizeCacheKey(request);
  if (!cacheKey) return;
...
sw.addEventListener('message', (event: ExtendableMessageEvent) => {
  const data = (event as ExtendableMessageEvent & { data?: any }).data;
  ...
  if (
    ...
    event.respondWith(
      fetch(event.request)
        ...
        .catch(() =>
          matchCachedResponse(event.request).then((cachedResponse) => {
